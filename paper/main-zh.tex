...
成立，并在唯一一条显式陈述的已发表假设（晶格上 $\pw=\Theta(\tw)$）下推广到任意收缩树——
全部在 Lean~4 + Mathlib 中形式化（约 2{,}700 行，无 \texttt{sorry}，\textbf{零自定义公
理}）。两个部件都经过认证。其一是\emph{宽度}下界：bramble 是 pathwidth 的组合对偶证书，经
一个初等的区间 Helly 论证完全自证——这是 Lean 中首个 pathwidth 下界理论。其二是宽度到代价
的联结：我们的 \emph{Contraction Rigidity} 定理证明，任何\emph{有能力}在割面达到满秩的网
络，对真代数子簇之外的一切门参数都满秩——不含概率，且"能力"前件经 Kronecker 路由约化为组
合的跨割面匹配——并且一个显式见证族在基准割面上实现了这种能力，对 generic 门取值封死低秩
（decision diagram）漏洞。在校准算例——53 线链的 $53\times53$ 深窗口时空 grid，对应
Sycamore-53 的\emph{割面尺度}——上，证书是\emph{零间隙}的：每个 stem 序宽度 $\ge 54$、总
...
\begin{itemize}
  ...
  \item \textbf{零间隙的认证校准算例（\S\ref{sec:endtoend}）。}在 $53\times53$ 深窗口时空
    ...
    （\texttt{sycamore53\_cost\_floor}），一个由跨割面匹配构造的见证族在 $53$ 比特割面上
    收缩刚性——对它，generic 意义下没有内维 $<2^{53}$ 的因子分解穿过割面
    （\texttt{sycamore53\_bond\_floor}）——而显式 sweep 达到下界：一个携带机器核验
    \emph{最优性证明}的找到的解，其 \texttt{\#print axioms} 只报告标准公理。
...
拿标尺读数与独立真值对表的实例族。而我们的标尺读数分毫不差。把模型实例化在 Sycamore-53 割
面尺度（$N=53$；53 线链的深窗口时空 grid），一条定理合取整个证书：
...
(iii)~每个序的总代价至少 $2^{54}$；(iv)~\emph{存在}一个由跨割面匹配构造的见证张量族在
$53$ 比特割面上收缩刚性——对该族，\texttt{no\_compression} 给出 generic 意义下没有内维低
于 $2^{53}$ 的因子分解穿过割面（\texttt{sycamore53\_bond\_floor}）。合取项 (iv) 是关于割
面纠缠容量的存在性陈述，不是对任何具体基准电路的秩断言。对该定理执行
\texttt{\#print axioms} 报告 \texttt{[propext, Classical.choice, Quot.sound]}。

保真度范围，直说：四个合取项谈论的并非同一个物理对象。合取项 (i)--(ii) 是基准参数
...
据位于深 regime 阈值之下。因此 (iii) 认证的是 Sycamore-53 割面尺度上的深 regime
\emph{校准}算例——不是 $(53,20)$ 的线链，也不是物理芯片的 $(2{+}1)$ 维晶格。合取项 (iv)
是 \emph{generic} 的：具体的 Sycamore 电路从离散集合中取单比特门，真子簇陈述不覆盖它——
...
$8\le N\le 28$ 时找到的宽度与认证尺度数值重合——在两种语义许可的粒度（$\pm1$）上一致，且
...
是 hyper-optimizer 的极限行为；更大预算能否收拢，正是绝对参照使之可量化的问题。最后这个
regime 用一句话就是标尺存在的理由。
...
化的张量比理论免费奉上的 stem 大 $2^{17}\approx 10^{5}$ 倍。在此搜索预算下，supremacy 类
架构上的搜索者不只是没能胜过 stem——它连\emph{找到} stem 都做不到，且缺口恰恰在最要紧的实
例上加剧。高出几个量级的预算能否收拢缺口是一个（有了标尺后可量化的）开放问题；无论如何，理
论免费奉上的 stem 是任何搜索都不应跌破的下限——这是我们所知的、支持"认证的 stem 感知排序"
的最强实践论据。
...
brickwork 电路确实具备把宽度下界变成刚性下界所需的纠缠能力。物理芯片具体的 $(2{+}1)$ 维
ABCD 耦合调度是我们尚未形式化的进一步保真度；刚性证书所消耗的——$\Omega(\min)$ 匹配尺度
与逐 bond 光锥可达性——已对 1D 链与 2D 芯片证明。
...
（stabilizer、真正低秩）可以也应当击穿它，而离散门集上的具体电路不被真子簇陈述覆盖。
(5)~零测度推论（$\Pr[\text{坍缩}]=0$）已陈述、尚未形式化。(6)~stem 收缩序与路径分解的等同
（\S\ref{sec:background}）、以及 \texttt{orderCost} 与收缩功的等同，是以 prose 论证的建模
对应；形式化的序空间\emph{就是}路径分解本身。(7)~实证对比（\S\ref{sec:empirical}）使用固
定且保守的搜索预算（$128$ 次重复）；观测到的漂移度量该预算下的搜索质量，非
hyper-optimizer 的极限行为。
...
匿名 artifact 档案（链接经投稿系统提供）包含完整 Lean 开发与实证流水线。Lean 开发：17 个
模块、约 2{,}700 行，\texttt{lake build} 构建于钉定 v4.30.0 的 Lean~4 / Mathlib；无
\texttt{sorry}、无 \texttt{axiom} 声明；检查脚本对本文引用的每条定理执行
\texttt{\#print axioms} 并断言输出恰为 \texttt{[propext, Classical.choice, Quot.sound]}。
