...
以 \emph{bond} 数计量的\emph{树}序，而认证下界量化在以顶点 bag 计量的 \emph{stem} 序上；在
grid 上 sweep 恰好实现 bond 宽度 $N+1$，而树序可以合法地低\emph{一}个单位（嵌套二分在列边
界处穿过，而 sweep 必须骑跨列中）——即 \S\ref{sec:limits} 的 $\pw$ 对 $\tw$ 之别，在数据中
现形。
...
实例 $128$ 次重复）。十四个实例中八个精确命中认证尺度；两个低于它一个单位（在 grid 上树序
可以合法地比 stem 类省一）；四个漂到 $+1$/$+2$ 之上——一种相对基准测试无法察觉、而标尺一眼
读出的质量缺口。}
\label{fig:gapplot}
\end{figure}

数据落在三个 regime，每个都在承担叙事工作。$8\le N\le 28$ 时搜索\emph{精确}命中认证尺度
——据我们所知，这是启发式收缩顺序第一次获得对其尺度的绝对确认，而非与其他启发式的比较。
$N\in\{6,40\}$ 处找到的树序低于 stem 线一个单位：理论预言的 stem/树间隙在野外被观测到。而
$N\in\{32,36,44,53\}$ 处固定预算的搜索漂到尺度\emph{之上} $+1$ 到 $+2$——在 $N=36$ 处返回
的顺序与认证线宣告可达的水平差了张量尺寸的 $4$ 倍，而只做相对评判的方法论永远不会标记它。
最后这个 regime 用一句话就是标尺存在的理由。
