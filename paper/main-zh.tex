...
浅层 regime 的负项衡量的是我们两条\emph{典范}见证的松弛（那里的最优 stem 是我们未枚举的时空
阶梯序），而非树胜过 stem 类。}
\label{tab:rqc}
\centering\small
\begin{tabular}{llrr}
...
1D 链，深（$L{=}2n$） & $n = 8..16$ & $n$ & $0,\,0,\,0$ \\
                      & $n = 20, 24, 28$ & $n$ & $+2,\,+2,\,+2$ \\
1D 链，$n{=}24$，深度 $L$ & $L = 6..22$（浅） & $\sim L{+}1$ & $-2$（见证松弛） \\
                      & $L = 26..46$（深） & $24$ & $0,\,+1,\,+2,\,+4$ \\
2D 芯片，ABCD & $16..36$ 比特，$8..12$ 周期 & $n$ & $+2 \;..\; +9$ \\
\hline
\end{tabular}
\end{table}

三条观察。第一，\textbf{深 regime 中没有任何找到的树胜过所展示的 stem}，全部二十个实例如此
——stem 论题在真实 RQC 图上被检验。第二，$n{=}24$ 的深度扫描干净地描出
$\min(n,\text{空间割})$ 的过渡，交叉点恰在空间扫描超过时间扫描处（$L\approx 26$）。第三——
也是最锋利的发现——在 Sycamore 类 2D 架构上，自由搜索返回的树比平凡的时间扫描 stem 高出
$+2$ 到 $+9$ 个单位：$6\times 6$ 芯片、$12$ 周期时，返回顺序物化的张量比理论免费奉上的
stem 大 $2^{9}\approx 500$ 倍。在 supremacy 类架构上，搜索者不只是没能胜过 stem——它连
\emph{找到} stem 都做不到；这是我们所知的、支持"认证的 stem 感知排序"的最强实践论据。
