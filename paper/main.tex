...
quantifies over \emph{stem} orders in vertex-bag semantics; on grids the sweep realizes
bond-width exactly $N+1$, and tree orders may legitimately sit \emph{one} unit lower
(nested dissection crosses column boundaries where the sweep must straddle them) --- the
$\pw$-versus-$\tw$ distinction of \S\ref{sec:limits}, made visible in data.
...
\begin{tikzpicture}[font=\scriptsize, xscale=0.145, yscale=0.62]
  ...
  \node[gray, right, align=left] at (44.7,-1.35)
    {trees may shave one unit\\($\pw$ vs $\tw$)};
...
family (cotengra hyper-optimizer, $128$ repeats per instance). Eight of fourteen instances
meet the certified scale exactly; two sit one unit \emph{below} it (tree orders
legitimately undercut the stem class by one on grids); four drift $+1$/$+2$ above ---
a quality deficit that relative benchmarking cannot detect and the yardstick reads off
immediately.}
\label{fig:gapplot}
\end{figure}

The data lands in three regimes, each doing narrative work. For $8 \le N \le 28$ the
search meets the certified scale \emph{exactly} --- to our knowledge the first time
heuristic contraction orders come with an absolute confirmation of their scale rather than
a comparison to other heuristics. At $N \in \{6, 40\}$ the found trees sit one unit below
the stem line: the theoretically-predicted stem/tree gap, observed in the wild. And at
$N \in \{32, 36, 44, 53\}$ the fixed-budget search drifts $+1$ to $+2$ \emph{above} the
scale --- at $N=36$ the returned order is a factor $4$ in tensor size away from what the
certified line says is achievable, and nothing in a relative-only methodology would ever
flag it. That last regime is the yardstick's raison d'\^etre in one sentence.
