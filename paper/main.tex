...
in Lean~4 over Mathlib ($\sim$2{,}200 lines, no \texttt{sorry},
...
machine-checked optimality certificate: every stem order has width $\ge 27$ and pays total
cost $\ge 2^{27}$, no low-rank factorization crosses the cut below inner dimension $2^{53}$
generically, and our explicit sweep achieves width $\le 54$. Finally we report a cautionary
tale: our first iteration cited three
...
\begin{itemize}
  \item \textbf{An absolute yardstick for TNCO (\S\ref{sec:framework},\S\ref{sec:model}).}
    ...
    $2^{\lceil k/2\rceil} \le \mathrm{cost}(o)$ for every order $o$), not merely a width
    statement.
  ...
  \item \textbf{The first absolutely-certified benchmark instance
    ...
    every stem order to width $\ge 27$ and total cost $\ge 2^{27}$
    (\lstinline|sycamore53_cost_floor|), fences the cut against factorizations of inner
    dimension $< 2^{53}$ for generic gate values (\lstinline|sycamore53_bond_floor|),
    while our explicit sweep achieves width $\le 54$ --- an optimality-gap certificate of
    a factor $\le 2$ in the exponent, with \texttt{\#print axioms} reporting standard
    axioms only.
