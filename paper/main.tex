...
orders). Every point lands in $[-1, +2]$: eight of fourteen meet the certified scale
exactly, two sit at $-1$ (inside the one-unit bag-vs-bond conversion allowance --- not
below the stem thesis), and four drift $+1$/$+2$ above --- a quality deficit that relative
benchmarking cannot detect and the yardstick reads off immediately.}
\label{fig:gapplot}
\end{figure}
...
refutation. Second, for $8 \le N \le 28$ the search meets the certified scale
\emph{exactly} --- to our knowledge the first time heuristic contraction orders come with
an absolute confirmation of their scale rather than a comparison to other heuristics.
Third, at $N \in \{32, 36, 44, 53\}$ the fixed-budget search drifts $+1$ to $+2$
...
methodology would ever flag it. That last regime is the yardstick's raison d'\^etre in one
sentence.
...
\hline
1D chain, deep ($L{=}2n$) & $n = 8..16$ & $n$ & $0,\,0,\,0$ \\
                          & $n = 20, 24, 28$ & $n$ & $+2,\,+2,\,+2$ \\
1D chain, very deep ($L{=}3n$) & $n = 16..28$ & $n$ & $0,\,+2,\,+4,\,+4$ \\
1D chain, $n{=}24$, depth $L$ & $L = 6..22$ (below threshold) & $\sim L{+}1$ & $-2$ (out of domain) \\
                          & $L = 26..46$ (deep) & $24$ & $0,\,+1,\,+2,\,+4$ \\
2D chip, ABCD & $16..49$ qb, $8..20$ cyc. & $n$ & $+2 \;..\; +17$ \\
\hline
\end{tabular}
\end{table}
...
the stem --- it fails to \emph{find} it, and the failure worsens exactly where the
instances matter most; that is the strongest practical argument for certified stem-aware
ordering that we know.
