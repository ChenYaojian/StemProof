...
$54=N+1$），且没有任何点越到线下。认证尺度是搜索随预算增长的\emph{吸引子}——从上方逼近、
从不穿越。最后这个 regime 用一句话就是标尺存在的理由。
...
不只是没能胜过 stem——它连\emph{找到} stem 都做不到，且缺口恰恰在最要紧的实例上加剧。更大
...
$4$ 个宽度单位。理论免费奉上的 stem，是搜索仅以预算的对数速度逼近的下限——这是我们所知
的、支持"认证的 stem 感知排序"的最强实践论据。
...
对应；形式化的序空间\emph{就是}路径分解本身。(7)~实证对比（\S\ref{sec:empirical}）报告固定搜索预算，并附 $128/512/4096$ 次重复的敏感
性扫描：grid 漂移随预算增长收拢到换算容差之内，2D 芯片缺口则持续存在（$4096$ 下仍
$+6$ 到 $+10$）——观测到的间隙度量所述预算下的搜索质量，非极限行为。
