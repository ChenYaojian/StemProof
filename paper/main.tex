...
sweeps ($128$ repeats per instance). Negative shallow-regime entries measure the slack of
our two \emph{canonical} witnesses (the optimal stem there is a spacetime staircase we do
not enumerate), not trees beating the stem class.}
\label{tab:rqc}
\centering\small
\begin{tabular}{llrr}
\hline
...
1D chain, deep ($L{=}2n$) & $n = 8..16$ & $n$ & $0,\,0,\,0$ \\
                          & $n = 20, 24, 28$ & $n$ & $+2,\,+2,\,+2$ \\
1D chain, $n{=}24$, depth $L$ & $L = 6..22$ (shallow) & $\sim L{+}1$ & $-2$ (witness slack) \\
                          & $L = 26..46$ (deep) & $24$ & $0,\,+1,\,+2,\,+4$ \\
2D chip, ABCD & $16..36$ qb, $8..12$ cyc. & $n$ & $+2 \;..\; +9$ \\
\hline
\end{tabular}
\end{table}

Three observations. First, \textbf{no found tree beats an exhibited stem anywhere in the
deep regime}, across all twenty instances --- the stem thesis, checked on genuine RQC
graphs. Second, the $n{=}24$ depth sweep traces the $\min(n,\text{spatial cut})$ transition
cleanly, with the crossover exactly where the space sweep overtakes the time sweep
($L \approx 26$). Third --- the sharpest finding --- on the Sycamore-class 2D
architectures the free search returns trees $+2$ to $+9$ units \emph{above} the trivial
time-sweep stem: at $6{\times}6$ chips with $12$ cycles the returned order materializes
tensors $2^{9} \approx 500\times$ larger than the stem that theory hands over for free.
On supremacy-class architectures the searcher does not merely fail to beat the stem ---
it fails to \emph{find} it, which is the strongest practical argument for certified
stem-aware ordering that we know.
